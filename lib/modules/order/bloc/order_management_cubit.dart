...
class OrderManagementCubit extends Cubit<OrderManagementState> {
  ...
  OrderManagementCubit(
      ...
      : super(const OrderManagementState(orders: [])) {
  ...
  void _initData() {
    var items = _orderLocalDataSource.getOrderManagement();
  ...
  Future<void> _fetchItems(int page) async {
    try {
      final DataState<Pagination<OrderModel>> results =
          await _checkoutRepository.getShopOrders(page);
      if (results is DataSuccess) {
        ...
        if (page == 1) {
          _orderLocalDataSource.saveOrderManagement(_pagination.data);
        }
      } else {
        emit(state.copyWith(
          orders: _pagination.data,
